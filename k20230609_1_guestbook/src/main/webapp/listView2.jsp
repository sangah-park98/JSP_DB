...
<%--
	...
	위의 문장은 EL을 사용하면 <c:set var="view" value="${guestbookList.list}"></c:set>와 같이 사용할
	수 있다.
	
	jstl if문 => c:if => else를 사용할 수 없다.
		=> else 처리가 필요하면 조건을 반대로 해서 별도의 if를 만들어 사용한다.
	<c:if test="${조건식}">
		조건식이 참일 경우 실행할 문장
	</c:if>
					...
					<table class="table" width="99%" border="1" align="center" cellpadding="5" cellspacing="0">
						<tr>
							<td>
								...
								<!-- 메모에 태그가 먹지않고 줄이 바뀌도록 replace 함수를 적용한다. -->
