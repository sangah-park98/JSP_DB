...
//	oracle은 필드 선언시 데이터 타입을 char(고정길이 문자열)로 선언하면 필드의 크기보다 입력된 문자열의 크기가
//	작을 때 남는 자리가 전부 공백으로 리턴된다.
//	out.println("수정할 글의 비밀번호 글자수: " + original.getPassword().length() + "<br/>"); // 20
//	out.println("공백을 제거한 수정할 글의 비밀번호 글자수: " + original.getPassword().trim().length() + "<br/>"); // 4
//	out.println("수정하기 위해 입력한 비밀번호 글자수: " + vo.getPassword().length() + "<br/>"); // 4
//  테이블을 설계할 때 char 대신 varchar2(가변길이 문자열)로 선언한다.
//	이미 테이블이 char로 설계된 상태라면 trim() 메소드로 불필요한 공백을 제거해서 사용한다.
