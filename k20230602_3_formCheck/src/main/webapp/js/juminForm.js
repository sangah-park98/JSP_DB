...
//	인수로 넘어온 form의 유효성을 검사하고 정상이면 true, 오류이면 오류 메시지를 출력하고 false를 리턴하는 함수
...
//	false를 리턴시킨다.
	/*
	let test = document.getElementsByName('j1')[0];
	if(test.value.trim().length == 0 || test[0].value == '') {
		// 메시지
...
//	주민등록번호 앞자리의 6글자가 입력되었나 검사해서 6글자가 입력되지 않았으면 에러 메시지를 출력하고
//	false를 리턴시킨다.
...
//	자바스크립트는 문자열끼리 덧셈 연산을 하면 문자열을 연결하지만 뺄셈, 곱셈, 나눗셈은 숫자로 바꿔서 연산한다.
