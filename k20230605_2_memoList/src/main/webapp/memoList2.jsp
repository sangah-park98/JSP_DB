...
<%
//	1. ---------------------------------------------------------------------------------------------------
//	페이징 작업에 사용할 8개의 변수를 선언한다. => 계산에 의한 값을 가져야 하는 변수는 0으로 초기화
...
//	2. ---------------------------------------------------------------------------------------------------
//	totalCount 변수의 테이블에 저장된 전체 글의 개수를 얻어와서 저장한다.
//	테이블에 저장된 글이 있으면 글의 개수를 얻어와서 ResultSet 객체에 저장하고 테이블에 저장된 글이
//	없으면 0을 얻어와서 ResultSet에 저장하므로 ResultSet 객체에 저장되는 데이터가 무조건 존재하기 때문에 
// 	굳이 if 써서 물어볼 필요가 없는 것
...
//	이전 페이지에서 currentPage가 넘어오지 않으면 currentPage는 null이다. null에 parseInt() 메소드를
...
//	그렇지 않으면 초기값으로 지정한 1이 유지되게 한다.
	try {
		...
		// 번호가 전체 페이지 개수보다 큰 값이 넘어왔다면 화면에 표시할 페이지 번호를 전체 페이지 개수로
		// 수정한다.
...
//	memoList.jsp에서 사용했던 select sql 명령을 limit를 사용해서 전체글을 모두 얻어오던 것을
//	현재 페이지에 출력할 내용만 얻어오게 수정한다.
