...
//	memoUpdate.jsp에서 넘어오는 수정할 글번호(idx), 돌아갈 페이지 번호(currentPage), 
...
//	수정하기 위해 입력한 비밀번호와 수정할 글의 비밀번호(rs.getString("password"))를 비교해서
//	일치하면 글을 수정한다.
	out.println("<script>");
	if(password.equals(rs.getString("password"))) {
		// 비밀번호가 일치하면 글을 수정한다.
		sql = "update memolist set memo= ? WHERE idx = ?"; //  "update memolist set memo= ?, name= ? WHERE idx = ?";
