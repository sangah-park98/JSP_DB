...
//   delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의 카테고리   
...
//   delete.jsp또는 update.jsp에서 호출되는 삭제 또는 수정할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 
...
//   delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의 카테고리
//   1건을 "삭제된 카테고리 입니다."로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드
   public void deleteCategory(int idx) {
      System.out.println("CategoryService 클래스의 deletCategory() 메소드 실행");
      SqlSession mapper = MySession.getSession();
...
//   delete.jsp에서 호출되는 삭제할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의 카테고리
//   1건의 deleteCheck 필드의 값을 "YES"로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드   
   public void deleteCheck(int idx) {
      System.out.println("CategoryService 클래스의 deletCheck() 메소드 실행");
      SqlSession mapper = MySession.getSession();
...
//   restore.jsp에서 호출되는 복구할 카테고리 번호를 넘겨받고 mapper를 얻어온 후 CategoryDAO 클래스의 카테고리
//   1건의 deleteCheck 필드의 값을 "NO"로 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드      
   public void deleteRestore(int idx) {
      System.out.println("CategoryService 클래스의 deletRestore() 메소드 실행");
      SqlSession mapper = MySession.getSession();
...
// 	update.jsp에서 호출되는 복구할 카테고리 번호를 넘겨받고 mapper를 얻어온 후
// 	CategoryDAO 클래스의 카테고리 1건을 수정하는 update sql 명령을 실행하는 메소드를 호출하는 메소드      
   public void update(CategoryVO vo) {
    System.out.println("CategoryService 클래스의 deletupdate() 메소드 실행");
    SqlSession mapper = MySession.getSession();
...
// 	report.jsp에서 호출되는 신고할 카테고리 번호를 넘겨받고 mapper를 얻어온 후
// 	CategoryDAO 클래스의 카테고리 1건을 deleteReport 필드의 값을 1증가(수정)하는 
