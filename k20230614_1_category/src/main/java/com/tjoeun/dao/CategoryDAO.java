...
import com.tjoeun.service.CategoryService;
...
//	그룹(gup)에서 조건을 만족하는 카테고리 출력 순서를 조정하기 위해 seq를 1씩 증가시키는 category.xml 파일의
...
//	CategoryService 클래스에서 호출되는 mapper와 삭제 또는 수정할 카테고리 번호를 넘겨받고 카테고리 1건을
//	 "삭제된 카테고리입니다." 로 수정하는 category.xml 파일의 update sql 명령을 실행하는 메소드	
...
//	CategoryService 클래스에서 호출되는 mapper와 삭제 또는 수정할 카테고리 번호를 넘겨받고 카테고리 1건의
...
//	CategoryService 클래스에서 호출되는 mapper와 신고 횟수를 1증가 시킬 카테고리 번호를 넘겨받고 카테고리 1건의
//	deleteReport 필드의 값을 1 증가시키는 category.xml 파일의 update sql 명령을 실행하는 메소드	
